...
-- get vehicle yaw in radians
-- ahrs:get_yaw_rad() was added in ArduPilot 4.6.  Older firmware provides
-- ahrs:get_yaw() instead, so fall back to it when the newer binding is absent.
function get_vehicle_yaw_rad()
  if ahrs.get_yaw_rad then
...
function init()
  ...
  if driver and driver:add_filter(-1, RECEIVE_FRAMEID) then
    initialised = true
    gcs:send_text(MAV_SEVERITY.INFO, "DJIR: mount driver started (yaw-compat)")
  else
    gcs:send_text(MAV_SEVERITY.CRITICAL, "DJIR: failed to connect to CAN bus")   
  end

  if mount.set_natively_supported_mount_target_types then
  ...
  if now_ms - last_set_attitude_ms > SET_ATTITUDE_INTERVAL_MS then
    ...
    if roll_deg and pitch_deg and yaw_deg then
      if yaw_is_ef then
        ...
        yaw_deg = wrap_180(yaw_deg - math.deg(get_vehicle_yaw_rad()))
      end
